...
function ProcessStep({ step, key }: { step: Step; key?: React.Key }) {
  ...
  return (
    <section className={`flex flex-col ${step.reverse ? 'md:flex-row-reverse' : 'md:flex-row'} items-center gap-gutter-desktop relative overflow-hidden`}>
      <img
      ...
      />
      <motion.div {...imageReveal} className="w-full md:w-1/2 image-scale-container golden-shadow bg-alabaster p-2 border border-raw-sienna/10 group overflow-hidden">
        <img
          alt={step.imageAlt}
          src={step.image}
      ...
      >
        ...
        <h2 className="font-serif text-3xl md:text-4xl text-obsidian font-medium">{step.title}</h2>
        <p className="font-body-md text-body-md text-on-surface-variant max-w-md leading-relaxed">{step.body}</p>
        <div className={`pt-4 border-t border-raw-sienna/10 flex items-center gap-4 ${step.reverse ? 'md:flex-row-reverse w-full md:w-auto' : 'w-fit'}`}>
          <span className="material-symbols-outlined text-raw-sienna select-none" style={{ fontVariationSettings: "'wght' 200" }}>{step.icon}</span>
          <span className="font-label-sm text-label-sm text-tertiary uppercase tracking-widest font-medium">{step.iconLabel}</span>
        </div>
      </motion.div>
    </section>
  );
    ...
    <div className="bg-canvas text-on-surface antialiased selection:bg-raw-sienna selection:text-canvas min-h-screen flex flex-col pt-20">
      <main className="flex-grow pt-12 pb-section-gap px-page-margin-mobile md:px-page-margin-desktop max-w-screen-2xl mx-auto w-full">
        <div className="relative overflow-hidden">
          ...
          >
            <span className="font-label-caps text-label-caps text-raw-sienna tracking-[0.25em] block mb-4 uppercase">Our Craft Method</span>
